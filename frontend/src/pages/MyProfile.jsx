...
const MyProfile = () => {
  ...
  const dobValue = userData?.dob === 'Not Selected' ? '' : userData?.dob || '';
        ...
        <div className='max-w-lg flex flex-col gap-2 text-sm pt-5'>
            ...
            {isEdit
                ? <label htmlFor='image' >
                    <div className='inline-block relative cursor-pointer'>
                        <img className='w-36 rounded opacity-75' src={image ? URL.createObjectURL(image) : profileImage} alt="" />
                        {!image && <img className='w-10 absolute bottom-12 right-12' src={assets.upload_icon} alt="" />}
                    </div>
                    <input onChange={(e) => setImage(e.target.files[0])} type="file" id="image" hidden />
                </label>
                : <img className='w-36 rounded' src={profileImage} alt="" />
            }
            ...
            <div>
                ...
                <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-gray-600'>
                    ...
                    {isEdit
                        ? <input className='max-w-28 bg-gray-50' type='date' onChange={(e) => setUserData(prev => ({ ...prev, dob: e.target.value }))} value={dobValue} />
